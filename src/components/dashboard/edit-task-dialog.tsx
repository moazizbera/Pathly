...
import { completeTask, deleteTask, updateTask, type TaskActionState } from "@/app/actions/dashboard";
...
type EditTaskDialogContentProps = {
  task: TaskRecord;
  activeRole: ActiveRole;
  availableRoles: SupportedRole[];
  onClose: () => void;
...
function EditTaskDialogContent({ task, activeRole, availableRoles, onClose }: EditTaskDialogContentProps) {
  ...
  useEffect(() => {
    if (formState.success) {
      const timer = globalThis.setTimeout(() => onClose(), 900);
